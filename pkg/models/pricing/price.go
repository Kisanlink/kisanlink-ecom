...
type Price struct {
	*base.BaseModel
	...
	// Pricing rules
	MinQuantity *float64 `json:"min_quantity" gorm:"type:decimal(10,2)"` // Minimum quantity for this price
	MaxQuantity *float64 `json:"max_quantity" gorm:"type:decimal(10,2)"` // Maximum quantity for this price
	...
	Metadata    map[string]interface{} `json:"metadata" gorm:"type:jsonb"` // Additional pricing metadata
}

// NewPrice creates a new Price instance
func NewPrice(entityID, entityType, orgID string, priceType PriceType, amount float64, currency Currency) *Price {
	return &Price{
		BaseModel:  base.NewBaseModel("PRICE", hash.Medium),
		EntityID:   entityID,
...
func (p *Price) IsValidForQuantity(quantity float64) bool {
	if p.MinQuantity != nil && quantity < *p.MinQuantity {
		return false
	}

	if p.MaxQuantity != nil && quantity > *p.MaxQuantity {
		return false
	}
...
type PriceTier struct {
	*base.BaseModel
	...
	MinQty    float64  `json:"min_qty" gorm:"type:decimal(10,2);not null"`
	MaxQty    *float64 `json:"max_qty" gorm:"type:decimal(10,2)"` // NULL means unlimited
	UnitPrice float64  `json:"unit_price" gorm:"type:decimal(10,2);not null"`
	Currency  Currency `json:"currency" gorm:"type:varchar(3);not null;default:'INR'"`
}

// NewPriceTier creates a new PriceTier instance
func NewPriceTier(priceID string, minQty, unitPrice float64, currency Currency) *PriceTier {
	return &PriceTier{
		BaseModel: base.NewBaseModel("TIER", hash.Small),
		PriceID:   priceID,
		MinQty:    minQty,
		UnitPrice: unitPrice,
		Currency:  currency,
...
func NewPriceRule(orgID, name, ruleType string) *PriceRule {
	return &PriceRule{
		BaseModel: base.NewBaseModel("RULE", hash.Medium),
		OrgID:     orgID,
		Name:      name,
		RuleType:  ruleType,
		IsActive:  true,
