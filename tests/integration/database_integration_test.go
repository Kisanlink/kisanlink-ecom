...
type DatabaseIntegrationTestSuite struct {
    suite.Suite
    testDB          *TestDatabase
    catalogRepo     catalogRepo.CatalogRepositoryInterface
    orderRepo       orderRepo.OrderRepositoryInterface
...
func (suite *DatabaseIntegrationTestSuite) TestOrderCRUDOperations() {
    ...
    retrievedOrder, err := suite.orderRepo.GetByID(ctx, order.ID)
    suite.NoError(err)
    ...
    // Verify update
    updatedOrder, err := suite.orderRepo.GetByID(ctx, order.ID)
    suite.NoError(err)
    ...
    orders, total, err := suite.orderRepo.List(ctx, filter, testutils.TestUserID, testutils.TestOrgID, 0, 20)
    suite.NoError(err)
    suite.GreaterOrEqual(total, int64(1))
    suite.NotEmpty(orders)

    // Test Delete Order
    suite.T().Log("Testing order deletion")
    err = suite.orderRepo.Delete(ctx, order.ID)
    suite.NoError(err)

    // Verify deletion
    deletedOrder, err := suite.orderRepo.GetByID(ctx, order.ID)
    suite.Error(err) // Should return error for deleted order
    suite.Nil(deletedOrder)
}
...
func (suite *DatabaseIntegrationTestSuite) testConcurrentOrderProcessing(ctx context.Context) {
    ...
    for i, status := range statuses {
        go func(index int, orderStatus orderModels.OrderStatus) {
            // Retrieve order
            currentOrder, err := suite.orderRepo.GetByID(ctx, order.ID)
            if err != nil {
                done <- err
                return
            }
