...
func (r *OrderRepository) GetByBuyerID(ctx context.Context, buyerID string, limit, offset int) ([]*order.Order, error) {
...
func (r *OrderRepository) GetBySellerID(ctx context.Context, sellerID string, limit, offset int) ([]*order.Order, error) {
	filter := base.NewFilter()
	filter.Group.Conditions = []base.FilterCondition{
		{
			Field:    "seller_id",
...
func (r *OrderRepository) CreateOrderWithItems(ctx context.Context, order *order.Order) error {
...
func (r *OrderRepository) createOrderItem(ctx context.Context, item *order.OrderItem) error {
...
func (r *OrderRepository) createOrderWithoutTransaction(ctx context.Context, order *order.Order) error {
	// Create the order first
	if err := r.Create(ctx, order); err != nil {
		return err
	}
